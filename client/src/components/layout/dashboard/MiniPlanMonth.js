...
import { getOneMonthSummary } from '../../../actions/postPlanResults';
import Spinner from '../../layout/Spinner';
import PlanLineGraph from '../plan/PlanLineGraph';
import smileyIconRound from '../../../img/smiley-icon-round.svg';

const MiniPlanMonth = ({ user, getOneMonthSummary, planResult: { summaryPlanResult1m } }) => {


  useEffect(() => {
    getOneMonthSummary(user._id);
  }, [getOneMonthSummary, user._id]);
   console.log("plan weekly header has", summaryPlanResult1m );
  let planBodyResults = "";
  // if(summaryPlanResult1wk.summaryStats){
    if(summaryPlanResult1m){
                        ...
                        <div className="flex flex-row overflow-hidden items-start">
                            <div className="flex flex-col items-center w-72">
                                ...
                                <p className="mt-1 text-sm text-gray-500 mb-0 text-center">
                                    about your eating these days. It’s mostly because of <strong>{summaryPlanResult1m.summaryStats.stayedontrack}</strong>.
                                </p>
                            </div>
                            <div className="flex flex-col flex-grow mb-3 px-4 py-3 sm:px-6 items-center">
                                <div className="w-full h-48 mb-20">

                                <PlanLineGraph data={summaryPlanResult1m.dates[0]} values={summaryPlanResult1m.values[0]} />
                                    {/* <Line
                                        data={data}
                                        width={100}
                                        height={30}
...
MiniPlanMonth.propTypes = {
  getOneMonthSummary: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, { getOneMonthSummary })(MiniPlanMonth);