...
import { getPastDayResults } from '../../actions/postPlanResults';


  //const PlanPastDay = ({ user, getPastDayResults, previousPlanResult: { previousPlanResults } }) => {
    const PlanPastDay = ({ user, getPastDayResults, todayPlanResult: { todayPlanResults } }) => {
  //  const PlanPastDay = ({ user, getPastDayResults, todayPlanResult: { todayPlanResults } }) => {

    useEffect(() => {
        getPastDayResults('60f976566a593a1c5f520819');
    }, [getPastDayResults]);
  // console.log("plan results has", planResults2);

   console.log("previous days plan results has", todayPlanResults);
   console.log("previous date plan results has", todayPlanResults[0]);
...
PlanPastDay.propTypes = {
    ...
    getPastDayResults: PropTypes.func.isRequired,
    previousPlanResult: PropTypes.object.isRequired,
  //  previousPlanResults: PropTypes.object.isRequired,
  ...
  const mapStateToProps = (state) => ({
    // planResult: state.planResult2,
     ...
     previousPlanResult: state.previousPlanResult,
   //  previousPlanResults: state.previousPlanResults,
    isAuthenticated: state.auth.isAuthenticated,
    user: state.auth.user
  });
  
  export default connect(mapStateToProps, { getPastDayResults })(PlanPastDay);